...
// RULE 7: First/Restart pending trailing.
// Trigger = base step + PendingStepTrail.
// Reset = base step from the current market.
// This produces one PendingStepTrail displacement per modification.
void TrailSpecialFirstStopOrders(){
   ...
   for(int i=OrdersTotal()-1;i>=0;i--){
      ...
      double baseStep=0.0;
      string family="";
      if(StringFind(c,"EAGOLD R1 FIRST",0)>=0){baseStep=FirstStep;family="R1 FIRST";}
      else if(StringFind(c,"EAGOLD R7 RESTART",0)>=0){baseStep=BasketRestartStep;family="R7 RESTART";}
      else continue;
      if(baseStep<=0.0)continue;
      double trigger=PointsToPrice(baseStep+PendingStepTrail);
...
int OnInit(){ArrayResize(g_r9ProcessedTickets,0);g_r9HedgeActive=false;g_panelInitialized=false;g_panelMinProfit=0.0;g_panelMaxLots=0.0;R9SeedExistingPositions();PanelUpdate();Print(EA_NAME," v0.080 initialized. R9=activation + exposure + balance cap, R5=robust directional basket close with light-basket protection, R6=2xSmartGrid1 trigger/reset MinDistance, R7=baseStep+PendingStepTrail trigger/reset baseStep.");CreateFirstOrdersIfFlat();PanelUpdate();return(INIT_SUCCEEDED);}
