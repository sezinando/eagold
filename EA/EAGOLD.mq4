...
double NextRecoveryLot(double previousLot){
   // RULE 3: progression is calculated ONLY from the LAST NEW ORDER ACTUALLY ACTIVATED.
   // Formula: last activated lot x Multiplier + LotIncrement.
   // If there are no open positions, the first lot is always Lot.
   if(CountDirectionPositions(OP_BUY)<=0 && CountDirectionPositions(OP_SELL)<=0) return(NormalizeLot(Lot));
   double next=(previousLot*Multiplier)+LotIncrement;
   return(NormalizeLot(next));
}

void BuyRecovery(){ if(SmartGrid1<=0.0) return; if(CountDirectionPositions(OP_BUY)<=0) return; if(CountDirectionPending(OP_BUY)>0) return; double lastPrice,lastLot; int lastTicket; if(!GetLatestActivatedPosition(OP_BUY,lastPrice,lastLot,lastTicket)) return; RefreshRates(); double trigger=NormalizePrice(lastPrice-PointsToPrice(2.0*SmartGrid1)); if(Bid>=trigger) return; double newStop=NormalizePrice(lastPrice-PointsToPrice(SmartGrid1)); double stopLevel=MarketInfo(Symbol(),MODE_STOPLEVEL)*Point; if(newStop<=Ask+stopLevel) return; double nextLot=NextRecoveryLot(lastLot); Print(EA_NAME," BUY RULE 2/3: lastTicket=",lastTicket," lastPrice=",DoubleToString(lastPrice,Digits)," lastLot=",DoubleToString(lastLot,DigitsLots)," trigger=",DoubleToString(trigger,Digits)," newStop=",DoubleToString(newStop,Digits)," nextLot=",DoubleToString(nextLot,DigitsLots)); SendPending(OP_BUYSTOP,newStop,nextLot,"EAGOLD BUY RECOVERY"); }
void SellRecovery(){ if(SmartGrid1<=0.0) return; if(CountDirectionPositions(OP_SELL)<=0) return; if(CountDirectionPending(OP_SELL)>0) return; double lastPrice,lastLot; int lastTicket; if(!GetLatestActivatedPosition(OP_SELL,lastPrice,lastLot,lastTicket)) return; RefreshRates(); double trigger=NormalizePrice(lastPrice+PointsToPrice(2.0*SmartGrid1)); if(Ask<=trigger) return; double newStop=NormalizePrice(lastPrice+PointsToPrice(SmartGrid1)); double stopLevel=MarketInfo(Symbol(),MODE_STOPLEVEL)*Point; if(newStop>=Bid-stopLevel) return; double nextLot=NextRecoveryLot(lastLot); Print(EA_NAME," SELL RULE 2/3: lastTicket=",lastTicket," lastPrice=",DoubleToString(lastPrice,Digits)," lastLot=",DoubleToString(lastLot,DigitsLots)," trigger=",DoubleToString(trigger,Digits)," newStop=",DoubleToString(newStop,Digits)," nextLot=",DoubleToString(nextLot,DigitsLots)); SendPending(OP_SELLSTOP,newStop,nextLot,"EAGOLD SELL RECOVERY"); }
...
int OnInit(){ Print(EA_NAME," v0.042 initialized. Rules R1-R6 active. Multiplier=",DoubleToString(Multiplier,2)," LotIncrement=",DoubleToString(LotIncrement,DigitsLots)); BuyCreateFirstOrder(); SellCreateFirstOrder(); return(INIT_SUCCEEDED); }
