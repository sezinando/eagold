#property strict
#property version   "0.088"
#property description "EAGOLD - BUY/SELL independent machines - Rules 1 to 10 + Global STOP Trailing + R10 Profit-Funded Partial Grid Reduction + Equity Panel"
...
void CreateR10VisualMarker(int targetDirection,double reducedLots,double realizedMoney,double beforeExposure,double afterExposure,double beforeGross,double afterGross){
   if(!EnableR10VisualMarker||reducedLots<Lot)return;
   RefreshRates();
   double price=NormalizePrice((Bid+Ask)/2.0);
   string side=(targetDirection==OP_BUY?"BUY":"SELL");
   string base=R10_MARKER_PREFIX+IntegerToString(GetTickCount())+"_"+IntegerToString(MathRand());
   string shadow=base+"_SHADOW";
   string main=base+"_TEXT";
   string text=StringFormat("R10 %s\n-%.2f LOT\n$ %.2f",side,reducedLots,realizedMoney);
   double shadowPrice=NormalizePrice(price+PointsToPrice(R10MarkerOffsetPoints));
   double textPrice=NormalizePrice(price+PointsToPrice(R10MarkerOffsetPoints+3.0));
   datetime stamp=TimeCurrent();
   if(ObjectCreate(0,shadow,OBJ_TEXT,0,stamp,shadowPrice)){
      ObjectSetString(0,shadow,OBJPROP_TEXT,text);ObjectSetString(0,shadow,OBJPROP_FONT,R10MarkerFont);ObjectSetInteger(0,shadow,OBJPROP_FONTSIZE,R10MarkerFontSize);ObjectSetInteger(0,shadow,OBJPROP_COLOR,clrBlack);ObjectSetInteger(0,shadow,OBJPROP_ANCHOR,ANCHOR_LEFT_UPPER);ObjectSetInteger(0,shadow,OBJPROP_SELECTABLE,false);ObjectSetInteger(0,shadow,OBJPROP_SELECTED,false);ObjectSetInteger(0,shadow,OBJPROP_HIDDEN,false);ObjectSetInteger(0,shadow,OBJPROP_BACK,true);
   }
   if(ObjectCreate(0,main,OBJ_TEXT,0,stamp,textPrice)){
      ObjectSetString(0,main,OBJPROP_TEXT,text);ObjectSetString(0,main,OBJPROP_FONT,R10MarkerFont);ObjectSetInteger(0,main,OBJPROP_FONTSIZE,R10MarkerFontSize);ObjectSetInteger(0,main,OBJPROP_COLOR,R10MarkerColor);ObjectSetInteger(0,main,OBJPROP_ANCHOR,ANCHOR_LEFT_UPPER);ObjectSetInteger(0,main,OBJPROP_SELECTABLE,false);ObjectSetInteger(0,main,OBJPROP_SELECTED,false);ObjectSetInteger(0,main,OBJPROP_HIDDEN,false);ObjectSetInteger(0,main,OBJPROP_BACK,false);
   }
   Print(EA_NAME," R10 VISUAL MARKER: ",side," reduced=",DoubleToString(reducedLots,DigitsLots)," realized=$",DoubleToString(realizedMoney,2)," exposure=",DoubleToString(beforeExposure,DigitsLots)," -> ",DoubleToString(afterExposure,DigitsLots)," gross=",DoubleToString(beforeGross,DigitsLots)," -> ",DoubleToString(afterGross,DigitsLots));
   ChartRedraw(0);
}
...
bool R10FindProfitFundedPair(int direction,int &profitTicket,double &profitLots,double &profitPerLot,int &lossTicket,double &lossLots,double &lossPerLot){
   profitTicket=-1;lossTicket=-1;profitLots=0.0;lossLots=0.0;profitPerLot=0.0;lossPerLot=0.0;
   int type=(direction==OP_BUY?OP_BUY:OP_SELL);double bestProfit=-1.0e100,worstLoss=1.0e100;
   for(int i=OrdersTotal()-1;i>=0;i--){
      if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;
      if(!IsEAGOLDOrder()||OrderType()!=type)continue;
      double lots=OrderLots();if(lots<Lot)continue;
      double money=OrderProfit()+OrderSwap()+OrderCommission();
      double perLot=money/lots;
      if(money>0.0&&money>bestProfit){bestProfit=money;profitTicket=OrderTicket();profitLots=lots;profitPerLot=perLot;}
      if(money<0.0&&money<worstLoss){worstLoss=money;lossTicket=OrderTicket();lossLots=lots;lossPerLot=perLot;}
   }
   return(profitTicket>0&&lossTicket>0&&profitLots>=Lot&&lossLots>=Lot);
}

bool Rule10ProfitFundedPartial(int targetDirection){
   if(!EnableR10PairReduction||R10PairMinProfit<=0.0)return(false);
   if(R10PairCooldownSeconds>0&&g_r10LastAction>0&&TimeCurrent()-g_r10LastAction<R10PairCooldownSeconds)return(false);
   int profitTicket=-1,lossTicket=-1;double profitLots=0.0,lossLots=0.0,profitPerLot=0.0,lossPerLot=0.0;
   if(!R10FindProfitFundedPair(targetDirection,profitTicket,profitLots,profitPerLot,lossTicket,lossLots,lossPerLot))return(false);
   double pairLots=NormalizeDouble(MathMin(MathMin(profitLots,lossLots),R10PairMaxLots),DigitsLots);
   if(pairLots<Lot)return(false);
   double expectedPairProfit=(profitPerLot+lossPerLot)*pairLots;
   if(expectedPairProfit<R10PairMinProfit)return(false);
   double beforeExposure=ExposureLots();
   double beforeGross=DirectionLots(OP_BUY)+DirectionLots(OP_SELL);
   double realizedProfit=0.0,realizedLoss=0.0;
   Print(EA_NAME," RULE 10 PAIR TRIGGER: side=",(targetDirection==OP_BUY?"BUY":"SELL")," profitTicket=",profitTicket," lossTicket=",lossTicket," pairLots=",DoubleToString(pairLots,DigitsLots)," expected=$",DoubleToString(expectedPairProfit,2)," exposure=",DoubleToString(beforeExposure,DigitsLots)," gross=",DoubleToString(beforeGross,DigitsLots));
   if(!CloseMarketOrderLots(profitTicket,pairLots,realizedProfit))return(false);
   if(!CloseMarketOrderLots(lossTicket,pairLots,realizedLoss)){
      Print(EA_NAME," RULE 10 PAIR WARNING: losing-side partial close failed after profitable partial close. Exposure was reduced, but pair was incomplete.");
      g_r10LastAction=TimeCurrent();
      return(true);
   }
   double afterExposure=ExposureLots();
   double afterGross=DirectionLots(OP_BUY)+DirectionLots(OP_SELL);
   double netRealized=realizedProfit+realizedLoss;
   if(afterExposure>beforeExposure+0.00001){Print(EA_NAME," RULE 10 PAIR SAFETY FAILURE: exposure increased. before=",DoubleToString(beforeExposure,DigitsLots)," after=",DoubleToString(afterExposure,DigitsLots));return(false);}
   g_r10LastAction=TimeCurrent();
   Print(EA_NAME," RULE 10 PAIR COMPLETE: side=",(targetDirection==OP_BUY?"BUY":"SELL")," reduced=",DoubleToString(pairLots,DigitsLots)," realized=$",DoubleToString(netRealized,2)," exposure ",DoubleToString(beforeExposure,DigitsLots)," -> ",DoubleToString(afterExposure,DigitsLots)," gross ",DoubleToString(beforeGross,DigitsLots)," -> ",DoubleToString(afterGross,DigitsLots));
   CreateR10VisualMarker(targetDirection,pairLots,netRealized,beforeExposure,afterExposure,beforeGross,afterGross);
   return(true);
}

bool Rule10Reduce(int targetDirection){
   if(!EnableR10Reduce||R10MinExposureLots<=0.0)return(false);
   if(Rule10ProfitFundedPartial(targetDirection))return(true);
   int opposite=(targetDirection==OP_BUY?OP_SELL:OP_BUY);double targetLots=DirectionLots(targetDirection),oppositeLots=DirectionLots(opposite);double exposure=targetLots-oppositeLots;
   if(exposure<R10MinExposureLots)return(false);if(oppositeLots<Lot)return(false);if(targetLots<=oppositeLots)return(false);
   double reduceLots=NormalizeDouble(MathMin(exposure,oppositeLots),DigitsLots);if(reduceLots<R10MinExposureLots)return(false);
   double beforeExposure=ExposureLots(),beforeGross=targetLots+oppositeLots;double targetRealized=0.0,oppositeRealized=0.0;
   ...
   if(!ReduceDirectionByLots(targetDirection,reduceLots,targetRealized))return(false);
   if(!ReduceDirectionByLots(opposite,reduceLots,oppositeRealized)){Print(EA_NAME," RULE 10 WARNING: opposite-side reduction failed. System will not add exposure.");return(false);}
   double afterExposure=ExposureLots(),afterGross=DirectionLots(OP_BUY)+DirectionLots(OP_SELL);if(afterExposure>beforeExposure+0.00001){Print(EA_NAME," RULE 10 SAFETY FAILURE: exposure increased. before=",DoubleToString(beforeExposure,DigitsLots)," after=",DoubleToString(afterExposure,DigitsLots));return(false);}
   double totalRealized=targetRealized+oppositeRealized;g_r10LastAction=TimeCurrent();Print(EA_NAME," RULE 10 REDUCE COMPLETE: exposure ",DoubleToString(beforeExposure,DigitsLots)," -> ",DoubleToString(afterExposure,DigitsLots)," gross ",DoubleToString(beforeGross,DigitsLots)," -> ",DoubleToString(afterGross,DigitsLots)," realized=$",DoubleToString(totalRealized,2));CreateR10VisualMarker(targetDirection,reduceLots,totalRealized,beforeExposure,afterExposure,beforeGross,afterGross);return(true);
}
...
int OnInit(){ArrayResize(g_r9ProcessedTickets,0);g_r9HedgeActive=false;g_panelInitialized=false;g_panelMinProfit=0.0;g_panelMaxLots=0.0;g_r10LastAction=0;R9SeedExistingPositions();PanelUpdate();Print(EA_NAME," v0.088 initialized. BUY/SELL KEEP-ALIVE active; R10 profit-funded partial grid reduction + symmetric fallback; GLOBAL STOP TRAILING; EQUITY + ACCUMULATED PROFIT panel.");CreateFirstOrdersIfFlat();PanelUpdate();return(INIT_SUCCEEDED);}
