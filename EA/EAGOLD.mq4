#property strict
#property version   "0.099"
#property description "EAGOLD - BUY/SELL independent machines - Rules 1 to 10 + Dynamic Recovery Step Multiplier + Persistent Global Variables"
...
input string R10MarkerFont="Segoe UI Semibold";
input int R10MarkerFontSize=10;
input color R10MarkerColor=clrYellow;
input double R10MarkerOffsetPoints=25.0;

// RULE 11 - Dynamic Recovery Step Multiplier
input bool EnableRecoveryStepMultiplier=true;
input double RecoveryStepMultiplier=1.15;
input double RecoveryStepMax=500.0;

// PANEL - main background position/height ONLY. Text remains fixed.
...
// PERSISTENCE - every scalar EA input/runtime state is mirrored to MT4 Global Variables.
string StateKey(string metric){return(STATE_PREFIX+Symbol()+"_"+IntegerToString(MagicNumber)+"_"+metric);}
void PersistAllState(){
   GlobalVariableSet(StateKey("MagicNumber"),MagicNumber);
   GlobalVariableSet(StateKey("Lot"),Lot);
   GlobalVariableSet(StateKey("Multiplier"),Multiplier);
   GlobalVariableSet(StateKey("DigitsLots"),DigitsLots);
   GlobalVariableSet(StateKey("LotIncrement"),LotIncrement);
   GlobalVariableSet(StateKey("MaxOpenLot"),MaxOpenLot);
   GlobalVariableSet(StateKey("TakeProfit"),TakeProfit);
   GlobalVariableSet(StateKey("SellProfit"),SellProfit);
   GlobalVariableSet(StateKey("BasketLoss"),BasketLoss);
   GlobalVariableSet(StateKey("SpreadLimit"),SpreadLimit);
   GlobalVariableSet(StateKey("WaitSeconds"),WaitSeconds);
   GlobalVariableSet(StateKey("FirstStep"),FirstStep);
   GlobalVariableSet(StateKey("MiniGrid1"),MiniGrid1);
   GlobalVariableSet(StateKey("SmartGrid1"),SmartGrid1);
   GlobalVariableSet(StateKey("RecoveryMinDistance"),RecoveryMinDistance);
   GlobalVariableSet(StateKey("MiniGrid2"),MiniGrid2);
   GlobalVariableSet(StateKey("SmartGrid2"),SmartGrid2);
   GlobalVariableSet(StateKey("PendingStepTrail"),PendingStepTrail);
   GlobalVariableSet(StateKey("BasketRestartStep"),BasketRestartStep);
   GlobalVariableSet(StateKey("MaxTrades"),MaxTrades);
   GlobalVariableSet(StateKey("EnableCloseBy"),EnableCloseBy?1.0:0.0);
   GlobalVariableSet(StateKey("BuyProgressionTolerance"),BuyProgressionTolerance);
   GlobalVariableSet(StateKey("EnableR9Hedge"),EnableR9Hedge?1.0:0.0);
   GlobalVariableSet(StateKey("R9ExposureTriggerLots"),R9ExposureTriggerLots);
   GlobalVariableSet(StateKey("R9TriggerLotMinimum"),R9TriggerLotMinimum);
   GlobalVariableSet(StateKey("R9HedgeFraction"),R9HedgeFraction);
   GlobalVariableSet(StateKey("R9BalanceCap"),R9BalanceCap);
   GlobalVariableSet(StateKey("EnableR10Reduce"),EnableR10Reduce?1.0:0.0);
   GlobalVariableSet(StateKey("R10MinExposureLots"),R10MinExposureLots);
   GlobalVariableSet(StateKey("EnableR10PairReduction"),EnableR10PairReduction?1.0:0.0);
   GlobalVariableSet(StateKey("R10PairMinProfit"),R10PairMinProfit);
   GlobalVariableSet(StateKey("R10PairMaxLots"),R10PairMaxLots);
   GlobalVariableSet(StateKey("R10PairCooldownSeconds"),R10PairCooldownSeconds);
   GlobalVariableSet(StateKey("EnableR10VisualMarker"),EnableR10VisualMarker?1.0:0.0);
   GlobalVariableSet(StateKey("R10MarkerFontSize"),R10MarkerFontSize);
   GlobalVariableSet(StateKey("R10MarkerOffsetPoints"),R10MarkerOffsetPoints);
   GlobalVariableSet(StateKey("EnableRecoveryStepMultiplier"),EnableRecoveryStepMultiplier?1.0:0.0);
   GlobalVariableSet(StateKey("RecoveryStepMultiplier"),RecoveryStepMultiplier);
   GlobalVariableSet(StateKey("RecoveryStepMax"),RecoveryStepMax);
   GlobalVariableSet(StateKey("PanelBackgroundX"),PanelBackgroundX);
   GlobalVariableSet(StateKey("PanelBackgroundY"),PanelBackgroundY);
   GlobalVariableSet(StateKey("PanelBackgroundHeight"),PanelBackgroundHeight);
   GlobalVariableSet(StateKey("PanelBottomY"),PanelBottomY);
   GlobalVariableSet(StateKey("PanelBottomX1"),PanelBottomX1);
   GlobalVariableSet(StateKey("PanelBottomX2"),PanelBottomX2);
   GlobalVariableSet(StateKey("PanelBottomX3"),PanelBottomX3);
   GlobalVariableSet(StateKey("PanelBottomX4"),PanelBottomX4);
   GlobalVariableSet(StateKey("g_panelMinProfit"),g_panelMinProfit);
   GlobalVariableSet(StateKey("g_panelMaxLots"),g_panelMaxLots);
   GlobalVariableSet(StateKey("g_panelInitialized"),g_panelInitialized?1.0:0.0);
   GlobalVariableSet(StateKey("g_r9HedgeActive"),g_r9HedgeActive?1.0:0.0);
   GlobalVariableSet(StateKey("g_r10LastAction"),(double)g_r10LastAction);
   GlobalVariableSet(StateKey("R9ProcessedCount"),ArraySize(g_r9ProcessedTickets));
   for(int i=0;i<ArraySize(g_r9ProcessedTickets);i++)GlobalVariableSet(StateKey("R9ProcessedTicket_"+IntegerToString(i)),g_r9ProcessedTickets[i]);
   GlobalVariableSet(StateKey("TIMESTAMP"),(double)TimeCurrent());
}

// RULE 12 - Exposure Telemetry. Observation/persistence only; it never blocks or changes orders.
...
void UpdateExposureTelemetry(){
   ...
   double gross=buyLots+sellLots;
   double net=MathAbs(buyLots-sellLots);
   double maxIndividual=0.0;
   int openPositions=0;
   for(int i=OrdersTotal()-1;i>=0;i--){
      ...
      openPositions++;
      if(OrderLots()>maxIndividual)maxIndividual=OrderLots();
   }
   double balance=AccountBalance();
   double equity=AccountEquity();
   double currentDD=MathMax(0.0,balance-equity);
   double ddEquityPct=0.0;
   if(equity>0.0)ddEquityPct=(currentDD/equity)*100.0;
   int buyRecovery=RecoveryLevel(OP_BUY);
   int sellRecovery=RecoveryLevel(OP_SELL);
   int recoveryLevel=MathMax(buyRecovery,sellRecovery);
   GlobalVariableSet(TelemetryKey("BUY_EXPOSURE"),buyLots);
   GlobalVariableSet(TelemetryKey("SELL_EXPOSURE"),sellLots);
   GlobalVariableSet(TelemetryKey("GROSS_EXPOSURE"),gross);
   GlobalVariableSet(TelemetryKey("NET_EXPOSURE"),net);
   GlobalVariableSet(TelemetryKey("MAX_INDIVIDUAL_LOT"),maxIndividual);
   GlobalVariableSet(TelemetryKey("OPEN_POSITIONS"),openPositions);
   GlobalVariableSet(TelemetryKey("CURRENT_DD"),currentDD);
   GlobalVariableSet(TelemetryKey("DD_EQUITY_PCT"),ddEquityPct);
   GlobalVariableSet(TelemetryKey("BUY_RECOVERY_LEVEL"),buyRecovery);
   GlobalVariableSet(TelemetryKey("SELL_RECOVERY_LEVEL"),sellRecovery);
   GlobalVariableSet(TelemetryKey("RECOVERY_LEVEL"),recoveryLevel);
   GlobalVariableSet(TelemetryKey("BALANCE"),balance);
   GlobalVariableSet(TelemetryKey("EQUITY"),equity);
   GlobalVariableSet(TelemetryKey("TIMESTAMP"),(double)TimeCurrent());
}
...
void PanelCreate(){
   ...
   ObjectSetInteger(0,bg,OBJPROP_CORNER,CORNER_RIGHT_UPPER);
   ObjectSetInteger(0,bg,OBJPROP_ANCHOR,ANCHOR_RIGHT_UPPER);
   ObjectSetInteger(0,bg,OBJPROP_XDISTANCE,PanelBackgroundX);
   ObjectSetInteger(0,bg,OBJPROP_YDISTANCE,PanelBackgroundY);
   ObjectSetInteger(0,bg,OBJPROP_XSIZE,280);
   ObjectSetInteger(0,bg,OBJPROP_YSIZE,PanelBackgroundHeight);
}
...
void PanelCreateBottomLabel(string id,int x,int y,color clr){
   string name=PANEL_PREFIX+id;
   if(ObjectFind(0,name)<0){
      ObjectCreate(0,name,OBJ_LABEL,0,0,0);
...
void PanelUpdate(){
   ...
   double buyLots=0.0,sellLots=0.0,buyProfit=0.0,sellProfit=0.0,maxIndividual=0.0;
   for(int i=OrdersTotal()-1;i>=0;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;if(!IsEAGOLDOrder())continue;int type=OrderType();if(type==OP_BUY){buyCount++;buyLots+=OrderLots();buyProfit+=OrderProfit()+OrderSwap()+OrderCommission();if(OrderLots()>maxIndividual)maxIndividual=OrderLots();}else if(type==OP_SELL){sellCount++;sellLots+=OrderLots();sellProfit+=OrderProfit()+OrderSwap()+OrderCommission();if(OrderLots()>maxIndividual)maxIndividual=OrderLots();}else if(type==OP_BUYSTOP)buyPending++;else if(type==OP_SELLSTOP)sellPending++;}
   double totalProfit=buyProfit+sellProfit,totalLots=buyLots+sellLots,netLots=MathAbs(buyLots-sellLots),equity=AccountEquity(),accumulated=EAGOLDAccumulatedProfit();
   double balance=AccountBalance(),currentDD=MathMax(0.0,balance-equity),ddEquityPct=equity>0.0?(currentDD/equity)*100.0:0.0;
   ...
   int row=0;
   PanelSet("TITLE","EAGOLD  v0.099",row++,clrWhite);PanelSet("SEP1","==============================",row++,clrSilver);
   ...
   // R10 remains operational but is intentionally hidden from the main panel to reduce visual pollution.
...
void CreateFirstOrdersIfFlat(){if(CountEAGOLDOrders()==0){RefreshRates();int b=SendPending(OP_BUYSTOP,Ask+PointsToPrice(FirstStep),Lot,"EAGOLD R1 FIRST BUY");int s=SendPending(OP_SELLSTOP,Bid-PointsToPrice(FirstStep),Lot,"EAGOLD R1 FIRST SELL");if(b>0||s>0)Print(EA_NAME," RULE 1: initial seeds created. BUY=",b," SELL=",s);return;}EnsureDirectionMachineAlive(OP_BUY);EnsureDirectionMachineAlive(OP_SELL);}
...
void CreateR10VisualMarker(int targetDirection,double reducedLots,double realizedMoney,double beforeExposure,double afterExposure,double beforeGross,double afterGross){if(!EnableR10VisualMarker||reducedLots<Lot)return;RefreshRates();double price=NormalizePrice((Bid+Ask)/2.0);string side=(targetDirection==OP_BUY?"BUY":"SELL");string base=R10_MARKER_PREFIX+IntegerToString(GetTickCount())+"_"+IntegerToString(MathRand());string shadow=base+"_SHADOW";string main=base+"_TEXT";string text=StringFormat("R10 %s\n-%.2f LOT\n$ %.2f",side,reducedLots,realizedMoney);double shadowPrice=NormalizePrice(price+PointsToPrice(R10MarkerOffsetPoints));double textPrice=NormalizePrice(price+PointsToPrice(R10MarkerOffsetPoints+3.0));datetime stamp=TimeCurrent();if(ObjectCreate(0,shadow,OBJ_TEXT,0,stamp,shadowPrice)){ObjectSetString(0,shadow,OBJPROP_TEXT,text);ObjectSetString(0,shadow,OBJPROP_FONT,R10MarkerFont);ObjectSetInteger(0,shadow,OBJPROP_FONTSIZE,R10MarkerFontSize);ObjectSetInteger(0,shadow,OBJPROP_COLOR,clrBlack);ObjectSetInteger(0,shadow,OBJPROP_ANCHOR,ANCHOR_LEFT_UPPER);ObjectSetInteger(0,shadow,OBJPROP_SELECTABLE,false);ObjectSetInteger(0,shadow,OBJPROP_SELECTED,false);ObjectSetInteger(0,shadow,OBJPROP_HIDDEN,false);ObjectSetInteger(0,shadow,OBJPROP_BACK,true);}if(ObjectCreate(0,main,OBJ_TEXT,0,stamp,textPrice)){ObjectSetString(0,main,OBJPROP_TEXT,text);ObjectSetString(0,main,OBJPROP_FONT,R10MarkerFont);ObjectSetInteger(0,main,OBJPROP_FONTSIZE,R10MarkerFontSize);ObjectSetInteger(0,main,OBJPROP_COLOR,R10MarkerColor);ObjectSetInteger(0,main,OBJPROP_ANCHOR,ANCHOR_LEFT_UPPER);ObjectSetInteger(0,main,OBJPROP_SELECTABLE,false);ObjectSetInteger(0,main,OBJPROP_SELECTED,false);ObjectSetInteger(0,main,OBJPROP_HIDDEN,false);ObjectSetInteger(0,main,OBJPROP_BACK,false);}Print(EA_NAME," R10 VISUAL MARKER: ",side," reduced=",DoubleToString(reducedLots,DigitsLots)," realized=$",DoubleToString(realizedMoney,2)," exposure=",DoubleToString(beforeExposure,DigitsLots)," -> ",DoubleToString(afterExposure,DigitsLots)," gross=",DoubleToString(beforeGross,DigitsLots)," -> ",DoubleToString(afterGross,DigitsLots));ChartRedraw(0);}
...
int OnInit(){ArrayResize(g_r9ProcessedTickets,0);g_r9HedgeActive=false;g_panelInitialized=false;g_panelMinProfit=0.0;g_panelMaxLots=0.0;g_r10LastAction=0;R9SeedExistingPositions();UpdateExposureTelemetry();PanelUpdate();PersistAllState();Print(EAGOLD," v0.099 initialized. R12 telemetry active; all scalar inputs/runtime state persisted in Global Variables; R10 remains operational but hidden from panel; R11 dynamic recovery step multiplier active=",EnableRecoveryStepMultiplier," multiplier=",DoubleToString(RecoveryStepMultiplier,2)," max=",DoubleToString(RecoveryStepMax,1),"; GLOBAL STOP TRAILING.");CreateFirstOrdersIfFlat();UpdateExposureTelemetry();PanelUpdate();PersistAllState();return(INIT_SUCCEEDED);}
void OnDeinit(const int reason){PersistAllState();PanelDelete();}
void OnTick(){Rule9DetectActivatedOrders();BuyMachine();SellMachine();CreateFirstOrdersIfFlat();TrailAllStopOrders();UpdateExposureTelemetry();PanelUpdate();PersistAllState();}
